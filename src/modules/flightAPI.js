...
router.get('/getAllFlights', (req, res) => {
    ...
    try {
        ...
        //查询数据库
        sql.query(`select * from flightmanage limit ?,?`, [
            offset,
            parseInt(pageSize),
        ])
            .then(([data]) => {
                ...
                //查询总记录数
                return sql
                    .query(`select count(*) as total from flightmanage`)
                    .then(([[{ total }]]) => {
...
router.get('/getHotFlight', (req, res) => {
     ...
     */
    sql.query('select * from flightmanage order by rand()<=0.5 limit 4;').then(
        (response) => {
...
router.get('/getData/:column', (req, res) => {
    ...
    console.log(column)
    sql.query(`select ${column} from flightmanage group by ${column}`).then(
        (response) => {
            let [array] = response
...
router.get('/getCityPort', (req, res) => {
    ...
    let { city } = req.query
    sql.query(
        `select departPortName from flightmanage where departCity='${city}' group by departPortName`
    ).then((response) => {
        let [array] = response
...
router.get('/getFlightDetail/:flightNo', (req, res) => {
    ...
    sql.query(`select * from flightmanage where flightNo='${flightNo}'`)
        ...
        .catch((error) => {
            console.log('查询不到该航班!errors:', error)
            res.json({
                code: 400,
...
router.get('/getFlightTickets', (req, res) => {
    ...
    sql.query(
        `select * from flightmanage where departCity='${departCity}' and arriveCity='${arriveCity}'`
    ).then((response) => {
        let [data] = response
        data.forEach((item) => {
            item.departTime = formatTime(item.departTime)
...
router.put('/flight/:flightNo', (req, res) => {
    ...
    try {
        ...
        sql.query(
            'update flightmanage set departCity=?,arriveCity=?,departPortName=?,arrivePortName=?,departTime=?,arriveTime=?,price=?,airlineCompanyName=?,aircraftType=?,seatCount=?,bookedCount=?,remainingCount=? where flightNo=?',
            [
                departCity,
                arriveCity,
                departPortName,
